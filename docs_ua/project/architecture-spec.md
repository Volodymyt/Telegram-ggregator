...
Сервіс працює як довгоживучий контейнеризований Python-процес. Він підключається до Telegram через Telethon, слухає нові повідомлення з джерел, зберігає стан повідомлень із джерел і подій у Postgres та публікує вибрані сигнали загроз в один цільовий канал.
...
flowchart LR
    ...
    Reader[Зчитувач повідомлень]
    Store[Сховище повідомлень]
...
- Зберігає щойно побачені повідомлення з джерел із початковим статусом обробки.
...
- Використовує Postgres як джерело істини для повідомлень із джерел, стану кандидатів і стану логічних подій.
...
- Зберігає довговічний зв'язок між повідомленнями з джерел і логічними подіями.
...
- Postgres є канонічним довговічним станом для повідомлень із джерел і логічних подій.
...
MVP має тримати шар зберігання сфокусованим на повідомленнях із джерел і логічних подіях.
...
Окрема join-таблиця для MVP не потрібна. Довговічний зв'язок між повідомленнями з джерел і логічними подіями - це `message_records.event_record_id`.
...
sequenceDiagram
    ...
    participant Store as Сховище повідомлень
    ...
    Telegram-->>Reader: нова подія повідомлення з джерела
    Reader->>Store: вставити повідомлення зі status=received
    alt вже існує
        Store-->>Reader: дублікат повідомлення з джерела
        Reader-->>Reader: зафіксувати в логах і зупинитися
    else новий запис
...
- `reading/`: обробники подій Telethon, нормалізація повідомлень із джерел, налаштування підписок на джерела.
