...
1. Implement [M0 Storage bootstrap: storage surface and Alembic scaffold](tasks/01_storage_surface_alembic.md) to lock the canonical storage package surface and Alembic scaffold on top of the runtime and config contracts.
2. Continue with [M0 Storage bootstrap: schema baseline for message and event records](tasks/02_schema_baseline.md) to define the baseline SQLAlchemy Core schema and the first Alembic revision for `message_records` and `event_records`.
3. Implement [M0 Storage bootstrap: repository boundary and persistence primitives](tasks/03_repository_boundary.md) to establish the minimum stable repository contract on the baseline schema.
4. Implement [M0 Storage bootstrap: startup readiness and migration hooks](tasks/04_startup_readiness_hooks.md) to add one storage initialization path that proves database reachability and migration health before workers start.
5. Finish with [M0 Storage bootstrap: verification coverage](tasks/05_verification_coverage.md) to protect migrations, readiness, and baseline persistence primitives with automated coverage.
...
- Sequencing constraint: [M0 Storage bootstrap: repository boundary and persistence primitives](tasks/03_repository_boundary.md) and [M0 Storage bootstrap: startup readiness and migration hooks](tasks/04_startup_readiness_hooks.md) start only after [M0 Storage bootstrap: schema baseline for message and event records](tasks/02_schema_baseline.md) is stable and must share the same engine, metadata, and migration contract.
- Downstream reference: [M0 Bootstrap, observability, and test harness](../04_bootstrap_observability_test_harness/story.md) must consume the storage readiness hook from this story instead of reimplementing database checks.
