...
- Config validation covers source and target identifier formats, queue sizes, `LOG_LEVEL`, `DRY_RUN`, normalization toggles, filter mode semantics including the documented `all`-mode consistency rule, and the dedicated login flow plus existing-session startup expectations.
