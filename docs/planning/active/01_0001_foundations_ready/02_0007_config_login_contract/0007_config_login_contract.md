...
- Validate startup-relevant filter and runtime semantics, including `mode`, `case_insensitive`, normalization toggles, queue sizes, and the no-Telegram `DRY_RUN` rule.
...
- Validation covers `LOG_LEVEL`, `DRY_RUN`, queue sizes, normalization toggles, filter mode, the no-Telegram `DRY_RUN` rule, and `all`-mode consistency for typed include rules.
