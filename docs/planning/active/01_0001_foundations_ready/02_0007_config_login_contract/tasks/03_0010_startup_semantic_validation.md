...
- Validate `LOG_LEVEL`, `DRY_RUN`, queue sizes, filter-mode values, and normalization-related toggles used at startup.
- Enforce the documented `all`-mode rule that all typed include rules share the same `event_type` and `event_signal`.
...
3. Enforce filter-mode semantics, including the `all`-mode consistency rule for typed include rules.
